...
class df_detector:
    ...
    def df_classification(folder_path, threshold=0.5, predict_proba=True):
        classifier_dict = {}
        ...
        for path_to_image in image_list:
            ...
            try:
                ...
                if predict_proba:
                    #print("predict probability for", os.path.basename(path_to_image))
                    pred = restore.model.predict(pd.DataFrame(x_input).T)[0][0]
                else:
                    #print("predicting boolean...")
                    pred = restore.model.predict(pd.DataFrame(x_input).T, verbose=1)[0][0]
                    if pred > threshold:
                        pred = 1
                    elif pred < threshold:
                        pred = 0
                classifier_dict[os.path.basename(path_to_image)] = pred
            except:
                print("could not extract face from", os.path.basename(path_to_image))
